...
Wire it in the app factory — defining `install_openapi` without calling it leaves the
default schema in place:

```python
def create_app() -> FastAPI:
    app = FastAPI(...)
    app.include_router(users.router, prefix="/users", tags=["users"])
    install_openapi(app)  # <- without this line, the custom schema never applies
    return app
```
